...
def analyze(path):
    ...
    if n >= 4096:
        ...
        if len(S) > 1:
            ...
            den = S[:-1].sum(axis=1)                        # per-frame spectral energy
            m = den > max(den.max() * 1e-3, 1e-6)           # DROP near-silent frames (an intro/coda pad
            pf = np.sqrt((np.maximum(diff, 0) ** 2).sum(axis=1))[m] / den[m]  # fade-in divides a tiny diff by ~0)
            flux = float(pf.mean()) if pf.size else 0.0
