...
| ~~P5b-reneg~~ ✅ | **Mid-session re-negotiation** (closes P5b's deferred gap — sequential DAW track activation now hears its part). A sink registered/removed AFTER the owner has started raises a flag; the feeder, at a safe boundary under `ctlMutex_`, quiesces+respawns the reader, re-streams the new union (`audio.stop`→`audio.start`), re-resolves sink columns and resets the per-sink epoch. Race-free fence across the new epoch: `evtQueuedSeq_` stays **monotonic** (never `store(0)` to race the lock-free `queue*`) with a feeder-set `evtEpochBase_`; the fenced value is `seq−base`, so straddling events at worst UNDER-count (→ `evt_late`, never an over-count wedge). Single instance / no sink NEVER re-negotiates → **byte-identical** (golden held). **Verified PI4B-0001**: golden 65770cc8 byte-identical, `late-sink-test` (mid-session sink reliably non-silent **15/15** after the capture-arms-on-union-settle fix), `tsan-shell` new `latesink` config (the re-neg's reader quiesce/respawn + fence baseline + sink-epoch racing the live threads) **TSan-clean**, and the full claim-heavy suite green end-to-end on a fresh device. New `late-sink-test.sh` wired into hw runners; `tsan-shell` gains `latesink`. Built via two workflows + a targeted reliability/test fix. | **high** (RT core) | **done** |
...
| ~~xformat~~ ✅ | **VST3↔AU project-move e2e** (closes the AU row's tracked follow-up). `tools/au-host` gains `--part`/`--save-state`/`--load-state` writing the SAME on-disk layout as the VST3 host (`[comp_len][comp=HP1+part+bundle][ctrl_len=0]`; `--load-state` wraps the comp into a ClassInfo plist so `au_apply_classinfo` strips the header + applies). New `cross-format-recall-test.sh` (macOS, in `hw-tests.sh`): **gates on (B)** — the AU loading a VST3-SAVED state renders to the SAME hash as an AU-native save+load (generation-independent: the render reads params/part, not the bundle's snapshot counter). (A) raw byte-identity is reported informationally (the bundle embeds a device-side per-snapshot generation/head, so exact bytes are device-dependent, not a format issue). **Verified PI4B-0001**: B equal across runs (e.g. hash 187409bb), A byte-identical when the snapshot is stable. *Also*: both hw runners gained a `recover()` (daemon restart + wait-claimable) used as preflight AND a **retry-on-rc=3** in `run()` — self-heals a test that finds the bus wedged from a prior claim-heavy test's cumulative cycling (e.g. `late-sink`→`part-param-iso`). | low | **done** |
