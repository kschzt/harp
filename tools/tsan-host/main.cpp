...
static void control_thread(HarpRuntime *rt, EventSource *src, bool isOwner,
    ...
    for (int t = 0; t < seconds * 1000 && g_run.load(); t++) {
        ...
        if (n % 7 == 0)
            rt->queueNote(src, 0x20900000u | ((60 + (n % 12)) << 8) | 0x50, base);
        if (n % 11 == 0)
            rt->queueNote(src, 0x20800000u | ((60 + (n % 12)) << 8) | 0x40, base);
...
int main(int argc, char **argv) {
     ...
     * source BEFORE releasing its handle (a last release destroys the runtime;
     * unregistering after would touch a freed runtime). unregisterSource on the
     * owner source / nullptr is a no-op. */
    for (size_t k = 0; k < handles.size(); k++) {
