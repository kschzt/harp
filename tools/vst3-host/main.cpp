...
static int run_multi_instance(VST3::Hosting::Module::Ptr &module, const VST3::Hosting::ClassInfo &ci,
                              ...
                              const std::vector<std::pair<int, std::vector<int>>> &score, bool realtime,
    ...
    while (done < total) {
        ...
        for (int k = 0; k < N; k++) {
            ...
            const std::vector<int> *chn = &notes; int xpose = ch;   /* --score: this channel's OWN line */
            for (const auto &sp : score) if (sp.first == ch) { chn = &sp.second; xpose = 0; break; }
            for (size_t ni = 0; ni < chn->size(); ni++) {
                if ((*chn)[ni] < 0) continue;   /* < 0 = REST */
                int64_t on_at = (int64_t)((double)ni * note_period * rate);
                int64_t off_at = on_at + (int64_t)(0.75 * note_period * rate);
                int pitch = (*chn)[ni] + xpose;
...
int main(int argc, char **argv) {
    ...
    /* --score: per-channel note SEQUENCES (in-device multitimbral) — each channel plays its
     * own line into the ONE main instance (channel==device part), captured on the main mix. */
    std::vector<std::pair<int, std::vector<int>>> score;
    ...
    for (int i = 2; i < argc; i++) {
        ...
        } else if (a == "--score") { /* IN-DEVICE MULTITIMBRAL: per-channel note SEQUENCES,
                                      * "ch:n,n,n;ch:n,n" — one main instance, channel==part,
                                      * live-summed on the main mix (bus 0). */
            std::string sc = next();
            score.clear(); alias_channels.clear();
            size_t pos = 0;
            while (pos < sc.size()) {
                size_t semi = sc.find(';', pos);
                std::string seg = (semi == std::string::npos) ? sc.substr(pos) : sc.substr(pos, semi - pos);
                size_t colon = seg.find(':');
                if (colon != std::string::npos) {
                    int ch = atoi(seg.substr(0, colon).c_str());
                    std::vector<int> ns; std::string nl = seg.substr(colon + 1); size_t p = 0;
                    while (p < nl.size()) {
                        ns.push_back(atoi(nl.c_str() + p));
                        size_t c = nl.find(',', p); if (c == std::string::npos) break; p = c + 1;
                    }
                    if (ch >= 0 && ch <= 15 && !ns.empty()) { score.push_back({ch, ns}); alias_channels.push_back(ch); }
                }
                if (semi == std::string::npos) break;
                pos = semi + 1;
            }
            if (score.empty()) die("--score wants \"ch:n,n;ch:n,n\"");
    ...
    if (!alias_channels.empty()) {
        ...
        return run_multi_instance(module, audio_ci, alias_channels, serial, rate, block, seconds,
                                  bpm, notes, note_period, chord, score, realtime, out_path, do_hash,
                                  do_json, expect_hash);
    }
    ...
    while (done < total) {
        ...
        for (size_t ni = 0; ni < notes.size(); ni++) {
            if (notes[ni] < 0) continue;   /* a note value < 0 is a REST (syncopation/space) */
            int64_t on_at = (int64_t)((double)ni * note_period * rate);
            int64_t off_at = on_at + (int64_t)(0.75 * note_period * rate);
            if (on_at >= (int64_t)done && on_at < (int64_t)(done + n)) {
                Event ev{};
