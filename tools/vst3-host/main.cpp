...
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr,
                ...
                "  --instances N   drive ONE multi-out main instance on N MIDI channels\n"
                "             0..N-1 (a note on channel i routes to device part i, §9.4);\n"
                "             bus 0 sums the parts and is captured -> a multitimbral mix.\n"
                "             (M3: no registry, no per-instance Part param — one claim, one owner.)\n"
                "  --aliases L     like --instances but with an explicit channel list,\n"
                "             e.g. --aliases 0,1,2,3 (one channel per part)\n"
                "  --serial S      serial to pin the device claim (default: env\n"
                "             HARP_DEVICE_SERIAL or PI4B-0001); multi-channel mode only\n"
