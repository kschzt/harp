 ...
 * 2/3/4 (name/group/path); device-section key 2 (device logs, §4.2/§14.4) per-record key 3
 * (msg free-text — may carry a serial/host/path token). Preserved verbatim: vid/pid, firmware,
 * engine (incl. engine-id + param-map-hash), protocol, latency-profile, boot count,
 * ump-group-map, part count, caps, per-entry slot/direction/host-paced flag, the array
 * length/order, the whole counters map (no PII), and per log-record the msc/level/tag (the
 * tag is machine-greppable — reveals whether, not what). Returns false on a malformed section
