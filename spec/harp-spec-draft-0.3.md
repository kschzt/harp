...
Audio is independent of both controllership and event contribution: a device renders **one stream**, and exactly one shell in a shared group is the *audio owner* that pulls it (by default the first to bind; handoff follows the controllership rule). Sibling shells produce silence on their audio bus — the user routes device audio from the owner's track. Coexisting class audio is OS-arbitrated as usual.
