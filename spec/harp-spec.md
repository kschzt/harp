...
Specification, Version 1.1.2 — 30 June 2026

| | |
|---|---|
| **Status** | **Version 1.1.2 — stable.** (audio.fx §8.8 added as a backward-compatible capability over the frozen 1.0 wire; 1.1.1–1.1.2 are editorial clarifications, no wire change.) Breaking changes are negotiated at `core.hello` (§5.4) and flow through HARP Enhancement Proposals (§18) |
...
> **Changes in 1.1.2** — **Editorial: §15 host-runtime model reconciled to the shipped architecture, no wire change.** §15.1/§15.2 still documented the retired P4/P5 model — *N shells sharing one device session by serial*, an *owner+attached* refcount, a *controller/contributor* multitimbral group with cross-shell event merge, and a process-global session registry. The reference shells retired all of that for **one private runtime per plugin instance, exactly one instance per device**: a shell binds a single unit (selected by serial via `selectDevice()`), multiple synths are multiple instances kept apart by *selection* (not a shared table), and the only process-global state is the §8.4 admission ledger (keyed by transport path). Multitimbrality is realized in the host — the DAW routes multi-channel MIDI into the one multi-out instance (channel N → part N, §9.4/§9.10) and routes the per-part output buses (§6.3; bus 0 main mix, buses 1..N parts) back to tracks. §15.1 ¶1–2 and §15.2 are rewritten accordingly; the controller-handoff / contributor-shell / channel-range-merge / audio-owner machinery is removed (with one writer per device, state ownership is unambiguous). The speculative per-machine daemon is also dropped: the embedded in-process runtime **is** the reference architecture, and the runtime↔shell coupling stays implementation-defined. No identity key, message, or wire byte changes.
>
> **Changes in 1.1.1** — **Editorial: operational clarifications, no wire change.** Pins the few operational details an independent implementer would otherwise reverse-engineer from the reference code, documenting *existing* behaviour. (a) **§8.7** RTP **payload types** are now stated — 96 single-packet, 97 multi-packet group — with the group's 4-byte slot-range sub-header `{ slot_offset, n_slots, total_slots, flags }`. (b) **§8.3** gives the ASRC quality-floor **measurement method** behind the T14 gate (worst-case SINAD ≥ 120 dB / ripple ≤ 0.01 dB over an 18–23 kHz sweep through the converter alone). (c) **§8.3** makes the **offline signal** explicit: offline is recognised from the *carrier* (host-paced over the reliable framed link, §8.7), not a flag. (d) **§16** documents the reference **per-peer connection penalty** (vs a rejected global token bucket) and states the normative invariant (no source denies others; thresholds deployment-tunable). (e) **§9.3** makes ramp interpolation **block-size-independent** (control-rate sub-steps within a block). (f) **§7.3** recommends a PTP **profile** (default / AES67 media) and BMCA, with HARP devices follower-only. No identity key, message, or wire byte changes.
>
> **Version 1.1** — **Audio input and effect processing (§8.8, `audio.fx`).** A device MAY now declare capability **`audio.fx`** and *process* host-supplied audio — an insert or send effect — not only synthesize. This is a backward-compatible **MINOR** addition over the frozen 1.0 wire (per the version policy, a MINOR bump adds optional capabilities; old peers are unaffected — a host that does not implement it routes no input, a pure instrument never claims it). The transport was already capable: the §8.2 audio frame has an H→D direction and `audio.start` already negotiates `active-slots-in`, and §8.3 host-paced mode already requires "output for exactly the input sample ranges supplied" — so **deterministic / offline effect processing through hardware needs no new mechanism** (it inherits `audio.deterministic`/`audio.offline-rate`, certifiable under T15). §8.8 adds: the capability that declares the intent so the host routes audio in; a **wet-only dry/wet contract** (the device emits processed signal only, the host mixes against the dry it holds locally — the dry never touches the transport); **round-trip latency reporting** for PDC; and the **bidirectional clock model** real-time effect operation needs (input and output both cross the clock boundary — reconciled to one domain via `net.ptp` where possible, else asynchronously resampled each way to the §8.3 ASRC floors). Internal effect feedback (a regenerating tail, a resonator network's coupling) MUST close *within* the device — a host-traversing loop incurs the full transport round-trip and is out of model; closing the loop in dedicated hardware at single-sample latency is the property that makes a tight-feedback effect a reason to put the engine in hardware at all.
...
> **Changes in 0.3.6** *(host-runtime model superseded in 1.1.2 — the reference retired the shared-session/multitimbral-group design described below for one private runtime per instance; see the 1.1.2 note)* — Host-runtime model clarified where the shell↔device mapping is many-to-one (§15.1, §15.2). A device is a single transport client (one session, one claim), but several shells MAY share that session — the runtime keys sessions by `(vendor, product, serial)` and a shell binding an already-bound unit *attaches* rather than opening a second claim the transport forbids. Arbitration is split: **state** stays single-writer (one *controller* owns the patch — `live/project`, params, `state.*`), but **events are not state** (a note is momentary input, not a persistent edit), so shells sharing a session form a *multitimbral group* — each assigned a disjoint §9.10 channel range, contributions merged in timestamp order. This is how one physical unit appears as several DAW instrument tracks. A multitimbral device exposes per-part output channels (§6.3) demultiplexed to each shell — every track gets its own audio — optionally alongside a summed main mix; a single-mix device falls back to one *audio owner* with silent siblings. No wire-protocol change — this constrains host behavior only; the reference runtime is embedded in-process (a process-global registry).
...
**Free-running (mode 0, default; REQUIRED wherever analog converters are in the streamed path).** The device master clock owns the stream; timestamps are device `(epoch, msc)`. The runtime MUST recover the device clock continuously (§7.3), maintain a fixed-depth elastic buffer, and resample asynchronously into the DAW clock domain — *including when nominal rates match*, because independent crystals always drift; "both at 48 kHz" is two clocks, not one. ASRC quality floors: passband ripple ≤ 0.01 dB, stopband attenuation ≥ 120 dB, and rate corrections slew-limited below audibility. **Measurement method (the T14 gate):** sweep a full-scale sine across the **upper band 18–23 kHz** — where a resampler diverges, near Nyquist — through the converter *alone* (no recovery loop), and require the *worst-case* SINAD over the sweep ≥ 120 dB and passband ripple ≤ 0.01 dB; the reference converter and its conformance test share one quality setting so the gate cannot drift from what ships. Path latency MUST be constant for the session and reported in DAW-domain samples for plugin delay compensation. Buffer re-anchoring after starvation MUST be surfaced as an event and counted (`stream_reanchors`, host section of the diagnostic bundle) — never silent.
...
Host-paced mode admits two further OPTIONAL capabilities: `audio.deterministic` — byte-identical output for identical state, event schedule, and SSI range, certifiable under T15 — and `audio.offline-rate` — willingness to be paced faster or slower than real time, which turns offline bounce *through hardware* into an ordinary host feature rather than a stunt. **Offline is signalled by the carrier, not a flag:** the host requests an off-real-time bounce by pacing the device over a path with *no real-time stream to wedge* — on the network binding, host-paced SSI ranges and rendered audio over the reliable framed link (§8.7) rather than RTP; the device infers offline from that carrier and renders the fenced event set deterministically (it does not arm the §8.3.1 fence bound and does not count `fence_timeouts`). A host-paced stream over a real-time media path (the USB audio endpoints) is the real-time case and is unaffected.
...
Mapping: the RTP **sequence number** supplies the loss/reorder detection the reliable bulk endpoint made unnecessary on USB; the RTP **timestamp** is the device stream position (device MSC, free-running, §8.3); one **SSRC** per (device, direction, stream-slot group); the RTP **payload type** is **96** for a single-packet frame (payload = the raw slot-interleaved floats) and **97** for a multi-packet *group* whose payload begins with a 4-byte sub-header `{ u8 slot_offset, u8 n_slots, u8 total_slots, u8 flags }` — each group packet carries a contiguous slot range `[slot_offset, slot_offset + n_slots)` of the `total_slots`-wide frame, so a frame wider than the path MTU splits across slot ranges and reassembles by slot (the RTP sequence number orders the group, a missing group is concealed as silence). Both are dynamic payload types in the RFC 3551 range. Payload is interleaved by stream slot (§6.3), `nsamples × slots`. `float32` MUST be supported; `int24` (L24) SHOULD be offered for AES67-bridge compatibility (§4.4). A lost or reordered frame is detected via the sequence number and surfaced through the existing discontinuity accounting (`audio_late_frames`, §8.2); loss MUST be counted, never silently concealed. Epoch/rate changes follow §8.6 (the SSRC MAY change at restart).
...
### 8.8 Audio input and effect processing (`audio.fx`)

§8.1–§8.7 describe a device that *produces* audio into the plugin. A device MAY instead — or additionally — *process* audio the host supplies: an insert or send effect, where the host's track audio travels H→D, the device transforms it, and the result returns D→H. The transport already carries this. The §8.2 audio frame has an H→D direction (`dirflags` bit 0) and `audio.start` already negotiates `active-slots-in` (key 3); §8.3 host-paced mode already requires that "the device MUST produce output for exactly the sample ranges the host supplies as input." What §8.8 adds is not a new pipe but the **capability that declares the intent**, the **dry/wet contract**, the **latency-reporting** an insert needs, and the **bidirectional clock model** that real-time effect operation — unlike a pure synth — requires.

**Capability `audio.fx`.** A device declares `audio.fx` when one or more of its engines transform `active-slots-in` into `active-slots-out`. The host surfaces such a device as an insert/send effect: it routes the plugin's input to the device's input slots and the device's output back into the plugin's processing context (§8.1 — the effect path is the plugin path, never the OS audio path). A device's effect role is declared by the presence of the `audio.fx` capability in identity (the §6.2 capability array): a device that processes host audio claims it, a pure instrument never does, and a device whose engines do both claims it alongside its note/parameter capabilities. There is no separate role field — a host reads the role from the capability array and the engine/parameter descriptors, not from a dedicated identity key. (The reference engine is a compile-time synth-*or*-fx build; coexisting note- and input-driven engines in one device are allowed but not exercised by the reference.) An N-in/M-out effect is expressed directly by `active-slots-in`/`active-slots-out` (e.g. a mono-in/stereo-out reverb declares one in slot and two out slots).

**Dry/wet is the host's.** A device claiming `audio.fx` MUST emit **wet (processed) output only**; it MUST NOT mix the dry input into its output. The plugin holds the dry signal locally and mixes the returned wet against it after delay-compensation. This is structural, not stylistic: it keeps the dry path off the transport entirely (the network/USB round-trip never delays or colors the musician's direct signal), and it makes the device's reported latency the wet path's PDC value with no ambiguity. A "100% wet" engine and a host "mix" control together express every dry/wet ratio; the device need not know the ratio.

**Latency and PDC.** An effect MUST report its constant contribution — input transport + engine + output transport — in DAW-domain samples, via `device-pipeline-samples` (host-paced) or the free-running path-latency report (§8.3). The host adds its own transport round-trip and presents the total for plugin delay compensation. An insert is usable only where that total fits the host's PDC budget; the honest floor of a *networked* effect is the transport round-trip, not the engine (§8.7) — a fabric engine resolving the transform in a handful of clocks does not shorten the cable. Hosts SHOULD surface the effect's reported round-trip so a user can place it on a bus where the latency is acceptable rather than discover it by feel.

**Host-paced / offline effect (deterministic).** No new mechanism is required for deterministic effect processing: host-paced mode (§8.3) already defines exactly the contract — output for precisely the input sample ranges supplied. A host-paced device consuming input slots and transforming them *is* a deterministic effect; with `audio.deterministic` it is byte-identical for identical state, event schedule, and input range (certifiable under T15), and with `audio.offline-rate` an offline effect bounce *through hardware* is an ordinary host feature. `audio.fx` only declares the intent so the host routes audio in; the determinism guarantees of §8.3 carry over unchanged.

**Free-running effect (real-time, bidirectional).** Real-time effect operation requires what §8.3's free-running mode did not: audio crossing the clock boundary in *both* directions concurrently. A device claiming `audio.fx` in free-running mode MUST:

1. reconcile both directions to one clock where the binding allows it (`net.ptp`, §7.3) — input and output then share a single domain and a single ASRC boundary — **or**, absent a shared clock, recover the host stream clock for the input direction and present the output on the device clock, asynchronously resampling each crossing to the §8.3 floors (passband ripple ≤ 0.01 dB, stopband ≥ 120 dB). A device SHOULD prefer the single reconciled clock: double-ASRC adds a second resampling stage and its latency, and the network binding's real-time audio is otherwise free-running (§8.7);
2. report the constant round-trip latency for PDC, and surface re-anchoring on either direction as a counted event (`stream_reanchors`), never silently;
3. **close any internal effect feedback path within the device.** A feedback loop that leaves the device, traverses the host, and returns (output → host → input) incurs the full transport round-trip per iteration and cannot be made stable or musical; such loops are out of model. Feedback that is part of the effect (a regenerating reverb tail, a resonator network's internal coupling, a delay's feedback) MUST live entirely inside the device, where its latency is the engine's, not the cable's. (This is the property that makes a tight-feedback effect a meaningful reason to put the engine in dedicated hardware at all: the loop closes in the fabric at single-sample latency, which neither a buffer-latency host plugin nor a block-latency accelerator can match — see Appendix D.)

`audio.fx` is additive and backward-compatible: hosts that do not implement it see an ordinary instrument advertisement and route no input; a device that is purely an instrument never claims it.
...
**Set** applies a value at its timestamp. **Ramp** interpolates linearly in normalized space from the value in effect at the event's timestamp to `target` at `end`; the device interpolates at no less than its declared control rate — in sub-block steps **independent of the audio block size**, so a block longer than one control tick is interpolated across control-rate sub-steps within the block and the rendered curve never depends on block size (the reference engine smooths in 32-sample sub-blocks ≈ 1.5 kHz at 48 k). This is what makes automation zipper-free *and* cheap — a DAW curve becomes a handful of ramps per block instead of a point per tick. A new set or ramp on the same (param, voice) supersedes any ramp in flight. Devices claiming `evt.param` MUST implement ramps.
...
Each plugin shell owns a **private runtime** for its device — its transport claim, discovery, session, time correlation, object store, and diagnostics. There is exactly **one shell instance per device**: the shell constructs its own runtime, with no shared image and no cross-instance registry, so a shell maps one-to-one to the unit it binds. A host MAY run several runtimes at once — one per open device — kept apart by *selection* (each binds a different unit by serial, §15.2), never by a shared session table. The one piece of process-global state is the §8.4 admission-control bandwidth ledger, shared per transport *path* so independent runtimes on the same physical link (one USB controller, one Ethernet segment) honour a single bandwidth budget. Crash isolation between a host's plugins is the host platform's responsibility (modern DAWs sandbox plugins in separate processes); a runtime that loses its host process loses no committed state (§10.3), and the shell re-establishes with full recall on reload (§12.3, §11.4). The runtime SHOULD run user-space (no kernel components beyond OS-provided class drivers; Windows runtimes MAY additionally expose HARP stream devices through an ASIO driver for non-plugin use, outside this specification's scope). The runtime–shell coupling is in-process in the reference and otherwise implementation-defined; the *behavior* contract here is what conformance tests observe.

A device is a single client of its transport: one session, one claim (§4). That claim belongs to the one shell instance bound to the unit — there is no second shell to share it with, and no *attach-to-an-already-bound-unit* path. Two synths in one host are two instances binding two units; `selectDevice()` (the `HARP_DEVICE_SERIAL` env, the bundle's usb-identity, or first-unclaimed) is what keeps them distinct, not a shared registry. The device's multiple *parts* are not multiple shells: the multi-out shell exposes the device's per-part output channels (§6.3) as separate buses — bus 0 the summed main mix, buses 1..N the per-part pairs — and the host routes the parts it wants to its tracks. For audio, the runtime owns a real-time transport path per device and delivers samples to the shell over lock-free shared memory; the shell's audio-thread contract is hard — no blocking calls, no allocation, constant reported latency — and starvation at any boundary is counted and surfaced, never absorbed silently. Two corollaries proven necessary in practice: control-plane traffic the host originates (parameter pushes, state operations) MUST yield to stream service — the host-side mirror of §9.2's overload rule — and while an *offline* rendering context MAY block on stream progress (offline bounce legitimately waits for the wire, §8.3), a real-time context MUST NOT. Each runtime performs the §8.4 admission control for its own device against the shared per-path ledger.

### 15.2 Arbitration

A device has one patch state (the `live/project` ref, its parameters, its snapshots — §10, §11), and the one shell instance bound to the device owns it. With a single instance per device, **state-write ownership is unambiguous** — there is no second shell to arbitrate against, no controller election, and no handoff. The device's own single-claim rule (§4, §12) already enforces this: a second host process cannot open a competing session while the first holds the claim. Non-binding observers — an external meter bridge or a librarian reading `state.refs` over its own diagnostic session — see the same content-addressed state (§11) but never write it.

**Events are not state.** A note or other §9 event is a momentary input, not a persistent edit. Multitimbrality is realized in the *host*, not by multiple shells: the DAW routes multi-channel MIDI into the one instance, and the channel selects the part (channel N → part N; a note self-encodes its part via the §9.4 part key / §9.10 UMP group-channel). This is how one physical unit appears as several DAW instrument tracks — a MIDI track per part feeding the one instance, and that part's output bus returning to the track — with no second shell, no shared session, and no cross-shell event merge. A device that declares a single part (no `evt.multitimbral`; identity key 12 absent ⇒ one part) is the degenerate case: one part, one track.

Audio follows the same one-instance model. A multitimbral device SHOULD expose **per-part output channels** in its channel map (§6.3); the shell presents them as separate output buses — bus 0 the summed **main mix**, buses 1..N the per-part stereo pairs — and the host routes each part bus to its track. The host streams only the parts it routes (`active-slots-out`, §8), so an unrouted part costs no bandwidth; the main mix with a single active part is identical to that part's output. A device MAY expose only the summed main mix (no per-part channels); then every routed part is heard in that one stream and per-track separation is a host-side concern. Coexisting class audio is OS-arbitrated as usual.
...
**Denial of service**: credit (§4.2.1) bounds memory; devices SHOULD rate-limit `state.snapshot` and `diag.bundle`. On the Ethernet/IP binding (§4.4) a device MUST additionally bound half-open connections and rate-limit connection attempts, since any node on the segment can reach it. The **normative invariant** is that no single source can deny service to others; concrete thresholds are deployment-tunable, not fixed by this spec. The reference device meets it with a **per-peer penalty** rather than a global connection-count token bucket: a source that repeatedly fails *before completing `core.hello`* is shed for a short window (a bounded penalty ring keyed by source IP), and a connection that completes hello is never penalized — a global count limit was evaluated and rejected because a token bucket false-sheds a legitimate client arriving right after a burst, whereas per-peer shedding targets only the abuser. Half-opens that never send hello are dropped on a short deadline.
...
D→H ctl  {0:1, 1:1, 2:"core.hello", 3:{0:[1,0], 1:{
            ...
            6:["harp-core","harp-recall","harp-stream","harp-class-audio","audio.host-paced",
               ...
               "evt.ump","diag.loopback.digital","fw.ab-slots","audio.fx"],
...
*End of Version 1.1.2. Earlier drafts of the ideas herein circulated as a product brief titled "Hardware Plugin Runtime"; this document is the open-standard formulation of that work.*
